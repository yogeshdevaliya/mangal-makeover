		...
		<div class="col-md-6">
			...
			<div class="auth-card">
				...
				<div class="auth-card-body">
					<form method="POST" action="{{ route('login') }}">
							@csrf
							<input id="email" type="email" class="form-input form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" placeholder="Your Email Address" required autofocus>
							...
							&nbsp;&nbsp;
							@if (Route::has('password.request'))
								<a class="form-btn btn-link" href="{{ route('password.request') }}">
									{{ __('Forgot Your Password?') }}
								</a>
							@endif
					</form>
